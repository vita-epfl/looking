...
path_jaad = "/data/taylor-data/Datasets/JAAD/images/"
path_joints = "/home/younesbelkada/output_pifpaf_force/"
path_out = "./data/JAAD/"

file = open("results_new.csv", "r")
file_out = open("ground_truth.txt", "w")
	...
	try:
		...
		if len(data_json)> 0:
			...
			if iou_max > 0.3:
				...
				line = path+','+data["names"][i]+','+str(bb_final[0])+','+str(bb_final[1])+','+str(bb_final[2])+','+str(bb_final[3])+','+name_head+','+str(data["Y"][i])+'\n'
				name_pic += 1
