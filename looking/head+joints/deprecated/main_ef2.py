...
model = LookingNet_early_fusion_2("../head/models/resnet_head_best.p", "../joints/models/looking_model_jaad.p", device).to(device)
...
loss = nn.BCELoss()
optimizer = torch.optim.SGD(model.final.parameters(), lr=0.001, momentum=0.9, weight_decay=0.0005)
...
for e in range(EPOCHS):
	...
	if ap > ap_max:
		ap_max = ap
		torch.save(model.state_dict(), "./models/model_early_stop.p")
	model.train()
	#break
