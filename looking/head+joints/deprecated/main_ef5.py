...
#model = LookingNet_early_fusion_4("../head/models/resnet_head_video_best.p", "../joints/models/looking_model_jaad_video_full_kps.p", device).to(device)
#model = LookingNet_early_fusion_sum("../head/models/resnet_head_video_best.p", "../joints/models/looking_model_jaad_video_full_kps.p", device).to(device)
...
losses = []
ap_max = 0
writer = SummaryWriter(log_dir="./runs7_sum")
for e in range(EPOCHS):
	...
	if ap > ap_max:
		ap_max = ap
		torch.save(model.state_dict(), "./models/model_es_res18_video_04.p")
	model.train()
	#break
