...
class Kitti_dataset(Dataset):
        ...
        if self.type == 'joints':
            ...
            for line in self.file:
                ...
                if line_s[2] == self.split:
                    ...
                    X = joints[:17]
                    Y = joints[17:34]
                    self.head_centers.append([X[0], Y[0], X[3], X[4]])
                    X_new, Y_new = normalize_by_image_(X, Y, height_=False, type_='Kitti')
                    if self.pose == "head":
                        X_new, Y_new, C_new = extract_head(X_new, Y_new, joints[34:])
                        tensor = np.concatenate((X_new, Y_new, C_new)).tolist()
                    elif self.pose == 'body':
            ...
            for line in self.file:
                ...
                if line_s[2] == self.split:
                    ...
                    Xs = joints[:17]
                    Ys = joints[17:34]
                    self.head_centers.append([Xs[0], Ys[0], Xs[3], Xs[4]])
                    self.heights.append(int(line_s[-2]))
            self.file.close()
            return X, Y
        elif self.type == 'heads+joints':
            X = []
            Y = []
            kps = []
            for line in self.file:
                ...
                if line_s[2] == self.split:
                    ...
                    Xs = joints[:17]
                    Ys = joints[17:34]
                    X_new, Y_new = normalize_by_image_(Xs, Ys, height_=False, type_='Kitti')
                    tensor = np.concatenate((X_new, Y_new, joints[34:])).tolist()
                    kps.append(tensor) 
                    Y.append(int(line_s[-1]))
                    self.heights.append(int(line_s[-2]))
            ...
            for line in self.file:
                ...
                if line_s[2] == self.split:
                    X.append(line_s[1])
                    x1, y1, w, h = float(line_s[3]), float(line_s[4]), float(line_s[5]), float(line_s[6])
                    tensor = [(x1/self.WIDTH), (y1/self.HEIGHT), (x1+w)/self.WIDTH, (y1+h)/self.HEIGHT]
                    #print(tensor)
                    #exit(0)
                    kps.append(tensor) 
                    Y.append(int(line_s[-1]))
    ...
    def eval_ablation(self, heights, preds, ground_truths, image=False, black_im=None):
        """
        ...
        if image==True:
            ids_correct = (ground_truths == np.round(preds))
            ids_not_correct = (ground_truths != np.round(preds))
            Xs = self.xs[ids_correct]
            for correct_points in Xs:
                #print(correct_points)
                head = (int(correct_points[0]), int(correct_points[1]))
                radius = int(0.5*(abs(correct_points[3]-correct_points[2])))
                black_im = cv2.circle(black_im, head, radius, (0,255,0), -1)
                black_im = cv2.circle(black_im, head, radius+1, (255,255,255), 1)

            Xs = self.xs[ids_not_correct]
            for not_correct_points in Xs:
                #print(not_correct_points)
                head = (int(not_correct_points[0]), int(not_correct_points[1]))
                radius = int(0.5*(abs(not_correct_points[3]-not_correct_points[2])))
                #print(radius)
                black_im = cv2.circle(black_im, head, radius, (0,0,255), -1)
                black_im = cv2.circle(black_im, head, radius+1, (255,255,255), 1)
    ...
    def evaluate(self, model, device, it=1, heights_=False):
        assert self.split in ["test", "val"]
        black_im = np.zeros((375, 1242, 3), dtype = "uint8")
        ...
        if self.type == 'joints':
            ...
            for i in range(it):
                ...
                if heights_:
                    heights = np.array(self.heights)[np.concatenate((idx_Y1, idx_Y0[:N_pos]))]
                    self.xs = np.array(self.head_centers)[np.concatenate((idx_Y1, idx_Y0[:N_pos]))]
                ...
                for x_test, y_test in data_loader:
                    ...
                    out_lab = torch.cat((out_lab.detach().cpu(), out_pred.view(-1).detach().cpu()), dim=0)
                if heights_:
                    ap_1, ap_2, ap_3, ap_4, distance = self.eval_ablation(heights, out_lab, test_lab, True, black_im)
                    aps1.append(ap_1)
                    aps2.append(ap_2)
                    aps3.append(ap_3)
                    aps4.append(ap_4)
                ...
                aps.append(ap)
            if heights_:
                cv2.imwrite('test_joints.png', black_im)
                return np.mean(aps), np.mean(accs), np.mean(aps1), np.mean(aps2), np.mean(aps3), np.mean(aps4), np.array(distances)
            else:
                return np.mean(aps), np.mean(accs)
        elif self.type == 'heads':
            ...
            if heights_:
                cv2.imwrite('test_heads.png', black_im)
                return np.mean(aps), np.mean(accs), np.mean(aps1), np.mean(aps2), np.mean(aps3), np.mean(aps4), np.array(distances)
            return np.mean(aps), np.mean(accs)
        else:
            model.eval()
            ...
            for i in range(it):
                ...
                aps.append(ap)
                if heights_:
                    ap_1, ap_2, ap_3, ap_4, distance = self.eval_ablation(heights, out_lab, test_lab, False)
                    aps1.append(ap_1)
                    aps2.append(ap_2)
                    aps3.append(ap_3)
                    aps4.append(ap_4)
...
class LOOK_dataset(Dataset):
    ...
    def __init__(self, name, split, type_, path_gt_txt, path_data, pose="full", transform=None, device=torch.device('cpu')):
        ...
        self.device = device
        self.name = name
        assert self.name in ['jack', 'nu', 'kitti']
    ...
    def preprocess(self):
        ...
        for line in self.file:
            ...
            if line_s[1] == self.split and int(line_s[-1]) != -1: # verify if it's the right split
                ...
                X_new, Y_new, height = normalize_by_image_(Xs, Ys, height_=True, type_=self.name.title()) # normalize according to the paper
                if self.pose == "head":
                    X_new, Y_new, C_new = extract_head(X_new, Y_new, joints[34:])
                    tensor = np.concatenate((X_new, Y_new, C_new)).tolist()
                elif self.pose == 'body':
